Describe 'Portfolio update preparation' {
    ...
    BeforeEach {
        ...
        $currentManifest = Get-Content -LiteralPath (Join-Path $reference 'components/powershell/deployment-validation/component.json') -Raw | ConvertFrom-Json
        $currentVersion = [string] $currentManifest.version
        & git -C $reference tag "component/deployment-validation/v$currentVersion"
        ...
        [ordered]@{
            ...
            consumers = @(
                [ordered]@{
                    ...
                    adoption = 'adopted'
                    components = @(
                        [ordered]@{ id = 'deployment-validation'; desiredVersion = $currentVersion }
                    )
    ...
    It 'plans without changing a consumer, branch, or worktree' {
        ...
        $result = @(& $updater -Component deployment-validation -Version $currentVersion -PortfolioRoot $portfolioRoot -RegistryPath $registryPath)
    ...
    It 'prepares one isolated local branch and leaves the active checkout unchanged' {
        $beforeHead = (& git -C $consumer rev-parse HEAD).Trim()
        $result = @(& $updater -Component deployment-validation -Version $currentVersion -PortfolioRoot $portfolioRoot -RegistryPath $registryPath -Operation Prepare -WorktreeRoot $worktrees -Confirm:$false)
        $result.Count | Should -Be 1
    ...
    It 'removes its temporary branch when the consumer is already current' {
        & $updater -Component deployment-validation -Version $currentVersion -PortfolioRoot $portfolioRoot -RegistryPath $registryPath -Operation Prepare -WorktreeRoot $worktrees -Confirm:$false | Out-Null
